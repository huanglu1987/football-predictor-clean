...
def find_ap_triplets_for_odds(company_odds: List[Tuple[str, float]], tolerance_ticks: int = 1):
    ...
    规则（近似等差）：
      ...
      - 枚举 10 个三元组组合
      - 三元组按赔率从小到大排序
      - d1 = b-a, d2 = c-b（按tick：两位小数*100）
      - 若 |d1 - d2| <= tolerance_ticks (默认1，即0.01) 则认为近似等差
      - 要求严格递增（v1 < v2 < v3）

    返回：
      - trunc_vals: 按公司顺序截断后的 5 个赔率
      - triplets_desc: 满足近似等差的三元组描述列表（公司名+截断赔率+d1/d2/|d1-d2|）
    """
...
def analyze_odds_ap_for_match(input_row: pd.Series, tolerance_ticks: int = 1):
    """
    对当前比赛（输入的15列赔率）：
      - 主胜5赔率、平局5赔率、客胜5赔率分别检查“近似等差递增三元组”
    """
    result = {}
...
# ───────── 页面标题 & Session 初始化 ─────────
st.title("⚽ 历史相似比赛 Top5 查看与导出（含模式计数 / gap等差 / 赔率近似等差）")
...
""", unsafe_allow_html=True)
    ...
    if export_rows:
        ...
        st.subheader("📤 导出所有比赛的历史相似 Top5（含模式计数 + gap等差 + 赔率近似等差）")
        render_compact_table(df_export_with_blank.head(20))

        st.download_button(
            "⬇️ 导出历史相似 Top5（CSV）",
            df_export_with_blank.to_csv(index=False).encode("utf-8-sig"),
            "all_matches_top5_history_with_counts_and_ap_and_odds_ap_tol001.csv",
            "text/csv",
        )
