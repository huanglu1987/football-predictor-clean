...
#   - 当前比赛序号、当前主队/客队
#   - 历史主队/客队
#   - 历史比赛序号、历史比赛结果（同样自动识别列名）、历史PRO_最终预测结果、历史PRO_gap、历史PRO融合模型预测结果、历史PRO融合模型_gap（4 位小数）
...
if not st.session_state.input_df.empty and "matcher" in st.session_state:
    st.subheader("🔍 历史相似比赛 Top5")

    df_odds = st.session_state.input_df[ODDS_COLS].copy()

    # 当前比赛的 PRO 模型输出（如需用于 curr_pair，可保留）
    ...
    for i in range(len(st.session_state.input_df)):
        ...
        # 构造 q_basic（传递 PRO_gap / PRO融合_gap / 融合信心 / 推荐总分 / pair）
        ...
        # 直接取 Top5（严格按 SimilarityMatcher 排序顺序）
        try:
        ...
        # 显示指定列：比赛序号、比赛结果、PRO_最终预测结果、PRO_gap、PRO融合模型预测结果、PRO融合模型_gap
