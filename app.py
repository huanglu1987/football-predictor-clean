# app.py  –  加入 CV / Range / Pdiff 特征版 + gap 差值模式展示（两列对齐表格 + 导出 + 全库模式匹配参考）
...
if not st.session_state.input_df.empty:
    ...
    for i in range(len(st.session_state.input_df)):
        ...
        st.markdown("**1️⃣ 历史相似比赛（基于 PRO_gap / PRO融合_gap 等数值特征）**")
        sims_basic = sims_basic.reset_index(drop=True)
        # 这里直接展示全部列，方便你自己挑字段看
        st.dataframe(sims_basic, use_container_width=True)

        # 2) 用这 5 场比赛的 PRO_gap / PRO融合_gap 计算当前比赛的 6 个模式
...
"""
        ...
        # 3) 第二层：用“数值特征 + 6 个模式”去和全库 2300 场做匹配 → 找强参考
        q_pattern = {
            "PRO_gap": q_basic["PRO_gap"],
            "PRO融合模型_gap": q_basic["PRO融合模型_gap"],
            "融合信心": q_basic["融合信心"],
            "推荐总分": q_basic["推荐总分"],
            "pair": q_basic["pair"],
            "PRO_pattern_0_1_2": pro0,
        ...
        st.markdown("**3️⃣ 模式匹配参考（对比 2300 场历史模式，自动标记强参考）**")
