...
if mode == '📁 上传文件':
    ...
    with st.form('manual_form', clear_on_submit=True):
        ...
        if st.form_submit_button('提交录入'):
            ...
            if vals is not None and len(vals)==num_features:
...
if not st.session_state.input_df.empty:
    ...
    # 构造 ensemble 输入，扩展 PRO 特征
    try:
        df_ens_in = pd.concat([st.session_state.input_df.reset_index(drop=True), df_pro[['average_gap','P0','P1','P2']].reset_index(drop=True)], axis=1)
        df_ens = predict_model_pro_ensemble(df_ens_in)
    except ValueError:
        # ensemble 特征不匹配时回退
        df_ens = pd.DataFrame({
...
if not st.session_state.input_df.empty and st.button('🔄 运行综合预测'):
    ...
    df_ens2_in = pd.concat([df_in.reset_index(drop=True), df_pro2[['average_gap','P0','P1','P2']].reset_index(drop=True)], axis=1)
    try:
        df_ens2 = predict_model_pro_ensemble(df_ens2_in)
        p_ens = df_ens2[['P(主胜)','P(平局)', 'P(客胜)']].values
    except Exception:
