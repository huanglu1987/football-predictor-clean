...
def load_soccer_model():
    ...
    # 手动指定权重: 主胜=1.0, 平局=0.9, 客胜=1.0
    w = np.array([1.0 if yi==0 else 0.9 if yi==1 else 1.0 for yi in y])
...
if not st.session_state.input_df.empty:
    ...
    for idx, row in st.session_state.input_df.iterrows():
        odds_raw = row.values.astype(float).tolist()  # 15 维原始赔率
        try:
            sims = st.session_state.matcher.match(odds_raw, top_k=3)
        except TypeError:
            # 如果 match 不支持 threshold 参数，简单调用
            sims = st.session_state.matcher.match(odds_raw, top_k=3)
        except Exception:
            sims = []
        if sims:
