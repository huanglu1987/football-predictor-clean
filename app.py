...
def load_soccer_model():
    ...
    w = np.array([1.0 if yi==0 else 1.15 if yi==1 else 1.0 for yi in y])
...
if not st.session_state.input_df.empty:
    ...
    for idx, row in st.session_state.input_df.iterrows():
        # 先尝试 threshold=0 放宽匹配
        try:
            sims = st.session_state.matcher.match(row.values.tolist(),
                                                  top_k=3,
                                                  threshold=0.0)
        except TypeError:
            # 如果 match 不支持 threshold 参数，直接试 top_k
            try:
                sims = st.session_state.matcher.match(row.values.tolist(),
                                                      top_k=3)
            except Exception:
...
if not st.session_state.input_df.empty and st.button("🔄 运行综合预测"):
    ...
    try:
        ...
        if all(col in df_meta.columns for col in meta_cols):
            p_meta = df_meta[meta_cols].values
        else:
