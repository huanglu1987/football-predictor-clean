...
if mode == "📁 上传文件":
    ...
    st.info("请按顺序粘贴各公司赔率：主胜 平局 客胜，输入完5行后点击提交录入")
    # 手动输入缓存
    for comp in company_order:
        st.session_state.setdefault(f"man_{comp}", "")
    # 输入框
    inputs = []
    for comp in company_order:
        inputs.append(st.text_input(comp, placeholder="2.05 3.60 3.50", key=f"man_{comp}"))
    if st.button("提交录入"):
        vals = []
        for comp in company_order:
            parts = st.session_state[f"man_{comp}"].split()
            vals.extend([float(x) for x in parts])
            # 清空输入
            st.session_state[f"man_{comp}"] = ""
