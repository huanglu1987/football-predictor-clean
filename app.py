...
def find_ap_triplets_for_odds(company_odds: List[Tuple[str, float]], tolerance_ticks: int = 1):
    ...
    规则（近似等差）：
      - 赔率先截断到两位小数（不四舍五入）
      - 三元组按赔率从小到大排序
      - d1,d2 以 tick 计（两位小数*100）
      - 若 |d1-d2| <= 1 (0.01) 则认为近似等差
      - 严格递增
    """
...
# ───────── 页面标题 & Session 初始化 ─────────
st.title("⚽ 历史相似比赛 Top5 查看与导出（含gap_sum_100）")
...
if not st.session_state.input_df.empty and "matcher" in st.session_state:
    st.subheader("🔍 历史相似比赛 Top5（含 gap_sum_100）")
    ...
    for i in range(len(st.session_state.input_df)):
        ...
        # gap 两列显示 4 位小数（gap_sum_100 为整数不处理）
...
""", unsafe_allow_html=True)
        ...
        # ===== 导出行（每条 Top5 都带同样的统计信息 + gap_sum_100）=====
