...
#   - 截断两位小数（不四舍五入）
#   - 找出所有等差三元组，优先展示公差最小的一组，并保留所有三元组
...
def compute_gap_ap(sims: pd.DataFrame, col: str):
    ...
    若存在多个三元组，则优先选择“公差最小”的那一组，并同时返回所有三元组。
    返回：
      - trunc_list: 截断后的前 5 个 gap 列表（按 Top5 原始顺序）
      - has_ap: 1 / 0
      - best_triplet: 若存在，返回 (a,b,c) 三个 float（公差最小）；否则 None
      - all_triplets: 所有等差三元组列表 [(a,b,c), ...]
    """
...
"""
        ...
        if pro_has_ap and pro_best_triplet:
            pro_best_str = "、".join(f"{v:.2f}" for v in pro_best_triplet)
            st.caption(
                f"PRO_gap Top5 截断两位小数后: {pro_trunc_str}；"
                f"存在等差递增三元组: 是（最小公差：{pro_best_str}）"
            )
            if len(pro_all_triplets) > 1:
                all_str = "；".join("、".join(f"{v:.2f}" for v in t) for t in pro_all_triplets)
                st.caption(f"PRO_gap 所有等差三元组: {all_str}")
        else:
            st.caption(
        ...
        pro_best_triplet_str = ""
        pro_all_triplets_str = ""
        if pro_has_ap and pro_best_triplet:
            pro_best_triplet_str = "|".join(f"{v:.2f}" for v in pro_best_triplet)
        if pro_all_triplets:
            pro_all_triplets_str = ";".join("|".join(f"{v:.2f}" for v in t) for t in pro_all_triplets)
        ...
        if ens_has_ap and ens_best_triplet:
            ens_best_str = "、".join(f"{v:.2f}" for v in ens_best_triplet)
            st.caption(
                f"PRO融合模型_gap Top5 截断两位小数后: {ens_trunc_str}；"
                f"存在等差递增三元组: 是（最小公差：{ens_best_str}）"
            )
            if len(ens_all_triplets) > 1:
                ens_all_str = "；".join("、".join(f"{v:.2f}" for v in t) for t in ens_all_triplets)
                st.caption(f"PRO融合模型_gap 所有等差三元组: {ens_all_str}")
        else:
            st.caption(
        ...
        ens_best_triplet_str = ""
        ens_all_triplets_str = ""
        if ens_has_ap and ens_best_triplet:
            ens_best_triplet_str = "|".join(f"{v:.2f}" for v in ens_best_triplet)
        if ens_all_triplets:
            ens_all_triplets_str = ";".join("|".join(f"{v:.2f}" for v in t) for t in ens_all_triplets)
