# app.py  –  历史相似比赛 Top5 查看与导出 + 历史 TOP5 的 6 个差值模式 + 模式计数体系
#
# 显示内容：
#   - 每场当前比赛：
...
def compute_pattern_counts_for_match(
    ...
    """
    按当前“最严格”定义，基于 6 个模式计算：
      - PRO 内部：equal_pro（Δ 相等）、diff1_pro（Δ 差1且有共同数字）
      - ENS 内部：equal_ens、diff1_ens
      - PRO vs ENS 交叉：equal_cross（Δ 相等）、diff1_cross（Δ 差1且两模式间无共同数字）
      - total_count / parity
    """
...
"""
    ...
    if export_rows:
        ...
        st.subheader("📤 导出所有比赛的历史相似 Top5（含球队名称与计数结果）")
        ...
        st.download_button(
            ...
            "all_matches_top5_history_with_counts.csv",
            "text/csv",
        )
