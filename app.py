...
# + 5家公司主/平/客赔率：等差递增三元组（截断两位，不四舍五入，列出具体三元组并统计组数）
...
DATA_FILE = BASE_DIR / "data" / "new_matches.xlsx"  # 你原文件中保留
...
# ---------- PRO_gap & ENS_gap 等差三元组工具（你原本已有） ----------
def compute_gap_ap(sims: pd.DataFrame, col: str):
    """
    对 Top5 某列 gap（PRO_gap / PRO融合模型_gap）：
      - 截断两位小数（不四舍五入）
      - 找出所有严格递增等差三元组
      - best_triplet 选择公差最小的那组
    返回：trunc_list, has_ap, best_triplet, all_triplets
    """
...
# ========== 新增：赔率（三元组等差）工具 ==========
def find_ap_triplets_for_odds(company_odds: List[Tuple[str, float]]):
    """
    company_odds: [(公司名, 赔率原始值), ...] 长度=5
    规则：
      - 赔率先截断到两位小数（不四舍五入）
      - 枚举 10 个三元组组合
      - 三元组按赔率从小到大排序，判断是否严格递增等差
    返回：
      - trunc_vals: 按公司顺序截断后的 5 个赔率
      - triplets_desc: 满足等差的三元组描述列表（包含公司名+截断赔率）
    """
...
def analyze_odds_ap_for_match(input_row: pd.Series):
    """
    对当前比赛（输入的15列赔率）：
      - 主胜5赔率、平局5赔率、客胜5赔率分别检查等差递增三元组
    返回一个 dict，包含展示/导出所需字段。
    """
    result = {}
    for outcome in ["主胜", "平局", "客胜"]:
        ...
        trunc_vals, trips = find_ap_triplets_for_odds(company_odds)
...
# ───────── 页面标题 & Session 初始化 ─────────
st.title("⚽ 历史相似比赛 Top5 查看与导出（含模式计数 / gap 等差 / 赔率等差）")
...
if not st.session_state.input_df.empty and "matcher" in st.session_state:
    ...
    for i in range(len(st.session_state.input_df)):
        ...
        # ========== 新增：5家公司赔率等差递增三元组（主/平/客）==========
        input_row = st.session_state.input_df.loc[i]
        odds_ap = analyze_odds_ap_for_match(input_row)

        st.markdown("**赔率等差递增三元组（截断两位小数，不四舍五入）**")
...
""", unsafe_allow_html=True)
        ...
        for _, r in sims_basic_full.iterrows():
            export_rows.append({
                ...
                "历史PRO_gap": r.get("PRO_gap", ""),
                "历史PRO融合模型预测结果": r.get("PRO融合模型预测结果", ""),
                "历史PRO融合模型_gap": r.get("PRO融合模型_gap", ""),
    ...
    if export_rows:
        ...
        st.subheader("📤 导出所有比赛的历史相似 Top5（含模式计数 + gap等差 + 赔率等差）")
        render_compact_table(df_export_with_blank.head(20))

        st.download_button(
            "⬇️ 导出历史相似 Top5（CSV）",
            df_export_with_blank.to_csv(index=False).encode("utf-8-sig"),
            "all_matches_top5_history_with_counts_and_ap_and_odds_ap.csv",
            "text/csv",
        )
