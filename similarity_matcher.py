...
- 在模式匹配场景（只传6个模式字段）下：
  - 先按匹配个数从多到少排序；
  - 若匹配个数相同，再按“模式距离”（例如 2-7 vs 2-8）从近到远排序。
"""
    ...
    def _compute_pattern_match(
        ...
        for idx, col in enumerate(self.pattern_cols_pro):
            ...
            if qp is None or qp == "" or col not in self.df.columns:
                continue
            qp_pair = _parse_inner_pair(qp)
            if qp_pair is None:
                continue

            col_vals = self.df[col].astype(str).fillna("")
            # exact match 计数
            match_counts += (col_vals == str(qp)).astype(int)

            # 距离计算
            for i_row, v in enumerate(col_vals):
                vp_pair = _parse_inner_pair(v)
                if vp_pair is None:
                    continue
                d = abs(qp_pair[0] - vp_pair[0]) + abs(qp_pair[1] - vp_pair[1])
                sum_dist[i_row] += d
                cnt_used[i_row] += 1
        ...
        # 文字等级 + 强参考标记（仍然基于 exact match 个数）
        levels = []
        strong_flags = []
